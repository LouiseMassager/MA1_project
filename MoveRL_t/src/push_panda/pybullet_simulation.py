...
import os
import pathlib
folder_path=str(os.path.abspath(os.path.join(os.getcwd(),'..')))+"/MA1project_ws/src/ma1_project/push_panda/"
...
class PybulletSimulation(Simulation):
	def __init__(
		...
		text_file_name:str="data_angle.txt",
	) -> None:
		
		self.sim=sim
		self.timeposarray=timeposarray
		
		self.panda =Panda(self.sim, block_gripper=True)
...
#		self.view_type("push")
	...
	def view_type(self,settings:str) -> None:
		if (settings=="aesthetic"):
			self.sim.place_visualizer(self.box_position, 0.9, 50.0, -25.0) #esthétique view
		elif(settings=="push"):
			self.sim.place_visualizer(self.box_position+np.array([0.1,0.0,0.25]), 0.5, 10.0, 20.0) #esthétique view
		else:
...
#	def ee_frontbox_position(self,box_position: np.array, box_size: float) -> np.ndarray:
#		return box_position-np.array([box_size/2 + self.ee_thickness,0.0,0.0])
...
#		print("target angles:"+str(target_angles))
	...
	#	self.sim.control_joints(self.panda_name, self.panda.joint_indices, target_angles, self.panda.joint_forces)
		self.sim.step()	#update simulation/render only after 3rd iteration
		self.sim.render()
		if self.publisher is not None:
			self.publish_info()
		#test_str="joint"+str()+"angle = "+str()+"%s"
	...
	def redo_simulation_from_saved_data(self,saveddatafile:str,period:float):
		f= open(folder_path+"datafiles/"+saveddatafile,'r')
	...
	def throw_box(self):
		...
		for elem in l:
#			self.set_robot_position(pos+np.array([elem,0,0]))
#			self.set_robot_position(pos+np.array([elem,0,0]),self.ee_objective_direction)
