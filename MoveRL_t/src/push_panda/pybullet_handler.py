...
class PyBulletHandler(object):
    ...
    def __init__(
        self, render: bool = False, n_substeps: int = 20, background_color: np.ndarray = np.array([199.0, 240.0, 255.0])#np.array([223.0, 54.0, 45.0])
    ) -> None:
...
#	self.physics_client.setAdditionalSearchPath(os.path.join("pandacopied.urdf"))
    ...
    def inverse_kinematics(self, body: str, link: int, position: np.ndarray, orientation: np.ndarray=None) -> np.ndarray:
 ...
 #           self.physics_client.changeVisualShape(self._bodies_idx[body_name], -1, textureUniqueId=texture_uid)
    ...
    def create_plane(self, z_offset: float, color: np.array) -> None:
